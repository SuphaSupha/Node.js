...
app.get("/gender/female", (req, res) => {
  const females = data.filter((arr) => arr.gender === "Female");
  const namesOfFemales = females.map(
    (arr) => `${arr.first_name}  ${arr.last_name}`
  );
  res.send(namesOfFemales);
});
